...
class MetricsConfig:
    ...
    out_csv: str = "eval_results_dense_3q.csv"
    out_summary_json: str = "eval_summary_dense_3q.json"

    # evaluator model
    eval_llm_model: str = "google/flan-t5-small"
    max_new_tokens: int = 64

    # only test first 3 questions for now
    num_questions: int = 3
...
def build_evaluator_llm(cfg: MetricsConfig):
    ...
    """
    print(f"Loading evaluator model: {cfg.eval_llm_model}")
    hf_pipe = pipeline(
...
def score_with_ragas(rows: List[Dict[str, Any]], cfg: MetricsConfig) -> pd.DataFrame:
    print("Building evaluator LLM...")
    evaluator_llm = build_evaluator_llm(cfg)

    print("Converting rows to RAGAS samples...")
    ...
    print(f"Running RAGAS on {len(rows)} questions...")
...
def main():
    ...
    rows = rows[:cfg.num_questions]
    print(f"Using first {len(rows)} questions for test run")
